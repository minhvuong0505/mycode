...
class MyInfo extends Component {
    ...
    setUsers(user){
        var _avatar = ""; // https://cdn.freetalk.info/uploads/
        if(user.data.avatar != null) {
            if(user.data.avatar.includes("http")) {
                _avatar = user.data.avatar;
            } else {
                _avatar = window.location.origin+`/uploads/${user.data.avatar}`;
            }
        }
        var _imgUrl = (<div className="preview_avatar" style={{borderRadius:'50%',height:'33vw', width: '33vw', position:'absolute',top:'0',left: '0', backgroundImage: `url(${_avatar})`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize:'cover'}} />);
        this.setState({
            user: user.data,
  ...
  thumbImages(e, index) {
      ...
      reader.onloadend = (e) => {      
          images[index]['url'] = (<div className="preview_avatar" style={{borderRadius:'50%',height:'33vw', width: '33vw', position:'absolute',top:'0',left: '0', backgroundImage: `url(${reader.result})`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize:'cover'}} />);
          this.setState({ images })
          ...
          $.ajax({
              ...
              success: function(response) {
                if(response){
                    ...
                    _thisMyInfo.onSubmit(null, true);
                } 
              },
              error: function(err) {
                if (err.response) {
    ...
    render() {
        ...
        if(user.avatar != null) {
            if(user.avatar.includes("http")) {
                avatar = (<div className="preview_avatar" style={{borderRadius:'50%',height:'33vw', width: '33vw', position:'absolute',top:'0',left: '0', backgroundImage: `url(${user.avatar})`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize:'cover'}} />)
            } else {
                avatar = (<div className="preview_avatar" style={{borderRadius:'50%',height:'33vw', width: '33vw', position:'absolute',top:'0',left: '0', backgroundImage: `url(${CDN_URL+user.avatar})`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize:'cover'}} />)
            }
        } else {
            avatar = "";
        }
        ...
        <div className="block-myform block-myinfo">
          <div className="block-title mypage-heading">
            <a className="back" onClick={this.handleClose} />
            <span className="title">내 정보 관리</span>
          </div>
          {this.renderErrors()}
          {this.renderMessage()}
          <div className="block-subtitle">기본 정보 수정</div>
          <div className="block-content">
            <div className="form-group">
              <label className="label" htmlFor="form1">닉네임</label>
              <input type="text" value={user.name || ''} name="name" className="form-control" style={{textAlign:'right'}} onChange={ e => this.onChange({ name:e.target.name, value:e.target.value}) } />
            </div>
            <div className="form-group">
              <label className="label" htmlFor="form2">아이디</label>
              <input type="text" name="username" value={user.username || ''} className="form-control" style={{textAlign:'right'}} onChange={ e => this.onChange({ name:e.target.name, value:e.target.value}) } disabled />
            </div>
          ...
          <div className="block-subtitle">프로필 사진 변경</div>
          <div className="block-content">
            <div className="addphoto">
              <div className="photo" style={{position:'relative'}} >
                    <input id="avatar_preview_input" type="file" name="file" onChange={(e)=>this.thumbImages(e,1)} style={{opacity:0,position:'absolute',width:'30vw',height:'30vw'}}/>
                    {avatar}
              </div>
              <div className="text-center">
                <button className="btn btn-inline btn-sm" onClick={this.removeAvatar} >기존 사진 삭제</button>
                <button className="btn btn-default btn-sm" onClick={this.addAvatar} >저장</button>
              </div>
            </div>
          </div>
          <div className="block-subtitle">한 줄 소개</div>
          <div className="block-content">
            <div className="form-group">
          ...
          {/* <div className="block-subtitle">비밀번호 변경</div>
          <div className="block-content">
            <div className="form-group">
