...
class MyRecipes extends Component {
    ...
    async loadData(){
        ...
        axios.get('/api/v1/post?page_id='+offset+'&page_size='+limit+'&sorts=-id&filters=status=1'+user_id)
        .then((res)  => {
        ...
        return <div>
            {hearts.map((heart,index) => {
                ...
                if(heart.cooking_representation != undefined) {
                    ...
                    img = (<img src={CDN_URL+`${cooking_representation[0]['images']}`} alt={heart.title} />);
                    }
                }
                ...
                <div className="img">
                    <Link to={`recipe/${heart.id}/detail`}>{img}</Link>
                </div>
                <div className="detail">
                    <strong className="title"><Link to={`recipe/${heart.id}/detail`}>{heart.title}</Link></strong>
                    <div className="info">
                    <p><label>조리시간</label><span>{heart.cooking_time}분</span></p>
                    <div>
                        <label>난이도</label>
                        <Difficult diff={heart.difficulty} />
                    </div>
                    </div>               
                </div>
              </div>)
