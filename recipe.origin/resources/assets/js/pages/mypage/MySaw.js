...
class MySaw extends Component {
    ...
    async loadData(){
        ...
        axios.get('/api/v1/reaction?embeds=post&filters=object_type=post,type=view,user_id='+user_id+',status=1&page_id='+offset+'&page_size='+limit+'&sorts=-id')
        .then((res)  => {
    ...
    handleRemove(){
      ...
      axios.delete('/api/v3/reactions/react',{ headers: {
             ...
             data: {
                object_id:ids, object_type:'Post', type: 'View', action:0, user_id: PROFILES.id
            }})
        .then(res => {
            if (res.status === 200) {
  ...
  renderMessage() {
    ...
    renderLists(){
        ...
        return <div> {hearts.map((heart,index) => {
            ...
            <div className="img">
            <Link to={`recipe/${heart.object_id}/detail`}>
            {img}</Link>
            </div>
