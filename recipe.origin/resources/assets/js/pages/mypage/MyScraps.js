...
class MyScraps extends Component {
    ...
    async loadData(){
        ...
        axios.get('/api/v1/reaction?embeds=post&filters=object_type=post,type=save,user_id='+user_id+',status=1&page_id='+offset+'&page_size='+limit+'&sorts=-id')
        .then((res)  => {
    ...
    renderLists(){
        ...
        return <div> {hearts.map((heart,index) => {
            ...
            <div className="detail">
                <strong className="title"><Link to={`recipe/${heart.id}/detail`}>{heart.post.title}</Link></strong>
                <div className="info">
