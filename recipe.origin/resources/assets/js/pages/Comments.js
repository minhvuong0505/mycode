...
class Comments extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            object_id:this.props.object_id || 8, //post_id,talk_id
    ...
    componentWillReceiveProps(nextProps){
        //console.log(nextProps.object_id);
        if(this.state.object_id != nextProps.object_id){
        ...
        axios.post('/api/v3/comments', comments, {headers:{
            ...
            .then(res => {
                if (res.status === 201) {
                    ...
                    this.loadData();
                    myCommentState.setState({comments: { content: ""}});
                    var _cntComment = parseInt($('.item-id-'+object_id+' .item-bottom .comment').text()) + 1;
                    $('.item-id-'+object_id+' .item-bottom .comment').text(_cntComment);
                    var _total = parseInt(this.state.total) + 1;
                    total = _total;
                    myCommentState.setState({total});
                } else {
                    if(res.status == 200){
						alert('다시 로그인해야합니다.')
            ...
            {list.map((child,i) => {
                ...
                return <li key={`child-${i}`} className="item-reply">
                    <div className="item-info">
                        ...
                        <div className="info">
                            <span className="icon"><img src="/html/images/icon/icon3.png" alt="img" /></span>
    ...
    render() {
        ...
        let frmComent 
        let style_class = '';         
        if(this.state.object_type === 'Talk') style_class = {padding:'0vw',display:'none'}
        else style_class = {padding:'0vw'}

        let cmt = '';
        if(!lists.length){
            return <main className="site-main">
            <div className={`block-comment block-comment-${this.state.object_id}`}  style={style_class}>  
            <div className="block-bottom" style={{position:'relative'}}>
            <strong className="title"><input type="text" name="content" value={comments.content}
            onChange={(e) => this.onChange({ name: e.target.name, value: e.target.value, options:{parent_id:0,object_type:this.state.object_type,object_id:this.state.object_id} })} style={{width:'82vw'}} className="form-control" placeholder="댓글을 입력해주세요" /></strong>
            <button className="btn" id="all" onClick={this.onSubmit}>등록</button>
          </div>
          </div>
          </main>
        }
        ...
        return (
            ...
            {this.renderMessage()}
              <div className="block-title"  style={{position:'relative'}}>
                ...
                <button className="close" onClick={e=> this.handleClose(this.state.object_id)} />
              </div>
              <ul className="list-comment">
               {lists.map((list,index) => {
                   ...
                   return <li key={`lists-${index}`} className="item">
                   <div className="item-info">
                     <div className="comment">
                       {list.content}
                     </div>
                   ...
                   <div id={`form-comment-${index}`} className="form-comment" style={{display:'none'}} >
                     <input type="text" name="content" value={this.state.comments.content} onChange={(e) => this.onChange({ name: e.target.name, value: e.target.value, options:{parent_id:list.id, object_type:list.object_type ,object_id:list.object_id} })} className="form-control" placeholder="댓글을 입력해주세요" />
              ...
              <div className="block-bottom" style={{position:'relative'}}>
                <strong className="title"><input type="text" name="content" value={this.state.comments.content}
                onChange={(e) => this.onChange({ name: e.target.name, value: e.target.value, options:{parent_id:0,object_type:this.state.object_type,object_id:this.state.object_id} })} style={{width:'82vw'}} className="form-control" placeholder="댓글을 입력해주세요" /></strong>
                <button className="btn" id="all" onClick={this.onSubmit}>등록</button>
              </div>                                             
            </div>
