...
class MyScraps extends Component {
    ...
    async loadData(){
        ...
        axios.get('/api/v1/reaction?embeds=post,user&filters=object_type=post,type=save,user_id='+user_id+',status=1&page_id='+offset+'&page_size='+limit+'&sorts=-id')
        .then((res)  => {
    ...
    handleRemove(){
        ...
        axios.delete('/api/v3/reactions/react',{ headers: {
               ...
               data: {
                  object_id:ids, object_type:'Post', type: 'Save', action: 0, author_id:1
              }})
          .then(res => {
              if (res.status === 200) {
                ...
                {hearts.map((heart,index) => {
                    ...
                    <div key={`heart-${index}`}  className="col-sm-4 item item-recipe">
                        <div className="item-fearture-product">
                            ...
                            <div className="photo">
                                <Link to={`recipe/${heart.id}/detail`}><img src={CDN_URL+`${JSON.parse(heart.post.cooking_representation)[0]['images']}`} alt="img" /></Link>
                            </div>
                            <div className="detail">
                                <h3 className="title"><Link to={`recipe/${heart.id}/detail`}>{heart.post.title}</Link></h3>
            ...
            <div className="container">
              ...
              <div className="block-gird-item">
                <div className="toobar">
                  <strong className="title">스크랩한 레시피</strong>
                  <div className="clearfix" />
                  <span className="total">총 <span>{total}개의</span> 스크랩한 레시피가 있습니다</span>
                  <ul className="sorby">
