...
class Page extends Component {
    constructor(props) {
        ...
        if(!PROFILES){
            if(window.location.pathname.includes("/edit")) {
                // browserHistory.push("/login?redirect_url=my_recipe");
                window.location.href = "/login?redirect_url=my_recipe";
            } else {
                // change code
                // browserHistory.push("/login?redirect_url=recipe_write");
      ...
      this.onSubmitTemp = this.onSubmitTemp.bind(this);
    ...
    register(e) {        
        ...
        recipes['content'] = recipes['content'].trim();
        if(!recipes['content'] || recipes['content'].length<5 ){
        ...
        if(recipes.id != undefined) 
        {
            ...
            .then(res => {
                ...
                if (res.status == 201 || res.status == 200) {
                    ...
                    // this.setState({ message: '레시피가 업데이트되었습니다!' });
            ...
            $.ajax({
                ...
                success(res, textStatus, xhr) {
                    ...
                    if (xhr.status == 201 || xhr.status == 200) {
                        ...
                        // this.setState({ message: '조리법이 등록되었습니다!' });
                        this.setState({ message: languages.alert_recipe_registered });
                        // browserHistory.push("/my_recipes");
                    } else {
                        return res; 
                    }
                },
        ...
        axios.get(`/api/v1/post?filters=id=${recipes.post_id}`)//${recipes['post_id']}
            .then(response => {
                if(response.data.result) {
                    if(response.data.result[0].user_id == PROFILES.id) 
                    {
    ...
    componentDidMount(){
        ...
        if(props != undefined && props.routeParams.id != undefined && window.location.pathname.includes("/recipe") && window.location.pathname.includes("/edit"))
        {
            this.getRecipe();
        }
        //get categories
    ...
    handleVideoChange(e, index) {
        ...
        if(this.isVideo(file.name) !== true){
            // alert('동영상 올리기');
            alert(languages.alert_upload_video);
            return false
        }
        ...
        if(file.size > 1024*1024*8){
            ...
            alert(languages.alert_video_file_size);
            return false;
        }
    ...
    handleImageChange(e, index) {
        ...
        if(this.isImage(file.name) !== true){
            // alert('이미지 업로드 중');
            alert(languages.alert_upload_image);
            return false;
        }

        //valid size
        if(file.size > 1024*1024*8){
            alert(languages.alert_image_file_size);
            // alert('파일 크기는 8MB를 초과 할 수 없습니다.');
            return false;
        }
    ...
    deleteImage(index) {
        ...
        const con = confirm(languages.confirm_delete);
        if(con !== true) return false;
    ...
    uploadAdd(e){
        ...
        const con = confirm(languages.confirm_add_order);
        if(con !== true) return false;
    ...
    renderUploads(index){
        ...
        if(imagePreviewUrl[index] && imagePreviewUrl[index]['type'] == 'img') {
            $imagePreview = (<img style={{height:'100%', width:'100%', display:"block", backgroundImage: `url(${imagePreviewUrl[index]['url']})`, backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "cover"}} />);
        ...
        {
            $imagePreview = (<img style={{height:'100%', width:'100%', display:"block", backgroundImage: `url(${CDN_URL+_imgUrl})`, backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "cover"}} />);
        }
        ...
        return (
            <div key={`tod-${index}`}>
                <div className="block-title _gray">
                    <span className="title">STEP {index}</span>
                </div>
                <div className="block-content">
                    <div className="row form-group">
                        <div className="col-xs-12 text-right">
                            <a className="btn btn-outline btn-camera" style={{display:"none"}}>
                                <input className="btn btn-outline btn-camera" 
                                    type="file"
                                    style={style} 
                                    id={`ip-${index}`}
                                    onChange={(e)=>this.handleCamera(e,index)} />
                            </a>
                            <a className="btn btn-outline btn-images">
                                <input className="btn btn-outline btn-images" 
                                    type="file"
                                    style={style} 
                                    id={`ip-${index}`}
                                    onChange={(e)=>this.handleImageChange(e,index)} />
                            </a>
                            <a className="btn btn-outline btn-video" style={{display: "none"}}>
                                <input className="btn btn-outline btn-video"
                                    type="file"
                                    style={style} 
                                    id={`ip-${index}`}
                    ...
                    <div className="box-photo">
                        <div className="photo">
                            ...
                            <button className="btn btn-outline btn-block btn-sm" onClick={(e) => this.deleteImage(index)} type="button">{languages.av}</button>
                        </div>
                        <div className="des">
    ...
    renderRespectation(index){
        ...
        <div>
            <div className="row form-group">
                <div className="col-xs-12 text-right">
                    <a className="btn btn-outline btn-camera" >
                        <input className="btn btn-outline btn-camera" 
                            type="file"
                            style={style} 
                            id={`ip-${index}`}
                    ...
                    <a className="btn btn-outline btn-images" style={{display:"none"}}>
                        <input className="btn btn-outline btn-images" 
                            type="file"
                            style={style} 
                            id={`ip-${index}`}
                    ...
                    <a className="btn btn-outline btn-video" href="javascript:$('.input_upload_video').slideToggle();" >
                        
                    </a>
                </div>
            </div>
            <div className="row form-group input_upload_video" style={_showVideoStyle}>
                <div className="col-xs-2"><label className="label">Video Url:</label></div>
                <div className="col-xs-10"><input className="form-control" type="text" name="videos" value={_videosUrl} placeholder="Video Url" onChange={ (e) => this.onChange({ name:e.target.name, value:e.target.value}) } /></div>
            </div>
            <ul className="add-photo">
                {this.state.thumbs.map((thumb, key) => {
                    
                    if(key == 0) lbal = (<button className="btn btn-default btn-block" type="button">{languages.av1}</button>)
                    else lbal = '';
                    ...
                    if(recipes['cooking_representation'][key]){
                        if(recipes['cooking_representation'][key]['images'] != undefined) {
                            if(thumb.url != undefined) {
                                ...
                                let img_url = CDN_URL+recipes['cooking_representation'][key]['images'];
                                img = (
                                    <div style={{display: "block", width:"152px", height: "152px", backgroundImage: `url(${img_url})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat : "no-repeat"}}></div>
                                );
                                thumbs[key] = {'url' : img};
                            }
                        } else {
                    ...
                    return (
                        <li key={`tb-${key}`}>
                            <div className="photo">
                                ...
                                {lbal}
                                <button style={{display:"none"}} className="btn btn-default btn-block">{languages.df}</button>
                            </div>
                            <button className="btn btn-outline btn-block" onClick={(e) => this.thumbDelete(key, e)}>{languages.del_img}</button>
                        </li>
                    );
                })}
            </ul>
            <div className="row form-group">
                <div className="col-xs-12">                                    
                    <textarea className="form-control" rows={5} value={recipes['content']} name="content" onChange={ e => this.onChange({ name:e.target.name, value:e.target.value, index}) } placeholder={languages.ct} />
                </div>
            </div>
        </div>
        )
    ...
    thumbDelete(index, e){
        ...
        if(thumbs[index]['url'] != undefined){
            const con = confirm(languages.confirm_text);
            if(con !== true) return false;
            delete recipes['cooking_representation'][index];
    ...
    thumbImages(e, index) {
        ...
        reader.onloadend = (e) => {            
            ...
            thumbs[index] = {'url': (<div style={{height:'152px', width:'150px', borderRadius: "5px", display: "block", backgroundImage: `url(${reader.result})`, backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "cover"}} ></div>)}
            this.setState({ 
                file: file,
                thumbs
            }); 
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response){
                        if(recipes['cooking_representation'] == undefined) {
                            recipes['cooking_representation'] = {};
                            recipes['cooking_representation'][0] = {};
                            recipes['cooking_representation'][0]['images'] = 'tmp/'+response.file;
                        } else {
                            var _cntIndex = Object.values(recipes['cooking_representation']).length;
                            recipes['cooking_representation'][_cntIndex] = {};
                            recipes['cooking_representation'][_cntIndex]['images'] = 'tmp/'+response.file;
                        }
                        // if(recipes['cooking_representation'][index] != undefined){
    ...
    renderSubmit(){
        return <div className="actions-bottom text-center">
            <button className="btn btn-outline" onClick={ (e) => this.onSubmitTemp} type="button">임시저장 <span>4</span></button>
