            ...
            <li className="item">
              ...
              {options.map((option, index) => {
                    if(option.parent_id == 37)
                        return <label key={`option-${index}`}>
                            <input type="checkbox" value={option.id} name="function" onChange={ e => this.onChange({ name:e.target.name, value:e.target.value}) }  />
                            <span>{option.title}</span>
                        </label> 
                })}
              </div>
            ...
            <li className="item">
              ...
              {options.map((option, index) => {
                    if(option.parent_id == 51)
                        return <label key={`option-${index}`}>
                            <input type="checkbox" value={option.id} name="cooking_time" onChange={ e => this.onChange({ name:e.target.name, value:e.target.value}) }  />
                            <span>{option.title}</span>
                        </label> 
                })}
              </div>
